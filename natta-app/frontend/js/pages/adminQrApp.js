...
export function initAdminQrApp(appUrl, showToast) {
  ...
  document.getElementById('btnDescargarQr')?.addEventListener('click', () => {
    ...
    link.download = 'natta-app-qr.png';
    link.href = img.src;
    link.click();
  ...
  document.getElementById('btnCompartirQr')?.addEventListener('click', async () => {
    ...
    if (navigator.share && img?.src) {
      try {
        ...
        const file = new File([blob], 'natta-app-qr.png', { type: blob.type || 'image/png' });
        const shareData = {
          title: 'App Complejo Educativo Natta',
          text: `Accedé a la APP del Complejo Natta: ${appUrl}`,
          url: appUrl,
        };
