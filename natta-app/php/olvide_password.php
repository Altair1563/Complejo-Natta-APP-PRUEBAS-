...
$error_msg = isset($_SESSION['error_olvide']) ? $_SESSION['error_olvide'] : '';
unset($_SESSION['error_olvide']);
$success_msg = isset($_SESSION['success_olvide']) ? $_SESSION['success_olvide'] : '';
unset($_SESSION['success_olvide']);
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body class="cover" style="background-image: url(../assets/img/fondo-principal-app.webp);">
    <form action="procesar_olvide.php" method="POST" autocomplete="off" class="full-box logInForm">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrf_token, ENT_QUOTES, 'UTF-8'); ?>">
        <p class="text-center text-muted"><i class="zmdi zmdi-account-circle zmdi-hc-5x"></i></p>
        <p class="text-center text-muted text-uppercase">Recuperar contraseña</p>
        <p class="text-center text-muted">Ingresa tu email y el DNI del alumno para recibir un enlace de restablecimiento.</p>

        <?php if (!empty($error_msg)): ?>
            <div class="alert alert-danger text-center"><?php echo htmlspecialchars($error_msg, ENT_QUOTES, 'UTF-8'); ?></div>
        <?php endif; ?>
        <?php if (!empty($success_msg)): ?>
            <div class="alert alert-success text-center"><?php echo htmlspecialchars($success_msg, ENT_QUOTES, 'UTF-8'); ?></div>
        <?php endif; ?>

        <div class="form-group label-floating">
            <label class="control-label" for="email">E-mail</label>
            <input class="form-control" id="email" type="email" name="email" required>
            <p class="help-block">El e-mail que tienes registrado</p>
        </div>
        <div class="form-group label-floating">
        ...
        <div class="form-group text-center">
            <input type="submit" value="Enviar enlace" class="btn btn-raised btn-danger">
        </div>
