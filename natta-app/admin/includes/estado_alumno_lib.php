...
function estado_alumno_bulk_set_doc_recibida(
    ...
    $whereCurso = $curso !== ''
        ? 'UPPER(TRIM(l.curso)) = ?'
        : 'UPPER(RIGHT(TRIM(l.curso), 2)) = ?';
    $filtroCurso = $curso !== '' ? $curso : $escuelaCodigo;
    ...
    $actualizados = 0;
    try {
        estado_alumno_stmt_bind($stmtUpsert, 'sis', [$contractVersion, $recibida, $filtroCurso]);
        $stmtUpsert->execute();
    ...
    if ($stmtBulk) {
        try {
            estado_alumno_stmt_bind($stmtBulk, 'iss', [$recibida, $filtroCurso, $contractVersion]);
            $stmtBulk->execute();
            $stmtBulk->close();
        } catch (Throwable $e) {
...
{
    return [
        'CJ' => ['1ACJ', '2ACJ', '2BCJ', '3ACJ', '3BCJ'],
        'HV' => [
            '1AHV', '1BHV', '1CHV', '1DHV',
            '2AHV', '2BHV', '2CHV', '2DHV',
            '3AHV', '3BHV', '3CHV', '3DHV',
        ],
        'JA' => ['1AJA', '2AJA', '2BJA', '3AJA', '3BJA'],
        'SC' => [
            '1ASC', '1BSC', '2ASC', '2BSC', '3ASC', '3BSC',
            '4ASC', '4BSC', '5ASC', '5BSC', '6ASC', '6BSC',
        ],
        'JN' => [
            '1AJN', '1BJN', '1CJN', '1DJN', '1EJN',
            '2AJN', '2BJN', '2CJN', '2DJN', '2EJN',
            '3AJN', '3BJN', '3CJN', '3DJN', '3EJN',
            '4AJN', '4BJN', '4CJN', '4DJN', '4EJN',
            '5AJN', '5BJN', '5CJN', '5DJN', '5EJN',
            '6AJN', '6BJN', '6CJN', '6DJN', '6EJN',
        ],
        'MB' => [
            '1NMB', '11MB', '12MB', '13MB', '14MB',
            '21MB', '22MB', '23MB', '24MB',
            '31MB', '32MB', '33MB', '34MB',
            '41MB', '42MB', '43MB',
            '51MB', '52MB', '53MB',
...
function estado_alumno_cargar_alumnos_activos_por_curso(
    ...
    $whereCurso = $curso !== ''
        ? 'UPPER(TRIM(l.curso)) = ?'
        : 'UPPER(RIGHT(TRIM(l.curso), 2)) = ?';
    $filtroValor = $curso !== '' ? $curso : $escuelaCodigo;
    ...
    if ($contractVersion !== '') {
        ...
        estado_alumno_stmt_bind($stmt, 'ss', [$contractVersion, $filtroValor]);
    } else {
        $sql = "SELECT
                    l.nro_legajo,
                    l.apellido_alumno,
        ...
        estado_alumno_stmt_bind($stmt, 's', [$filtroValor]);
    }
...
function estado_alumno_cargar_revision_contratos_por_curso(
    ...
    if ($contractVersion !== '') {
        ...
        estado_alumno_stmt_bind($stmt, 'sss', [$contractVersion, $contractVersion, $filtroValor]);
    } else {
        $sql = "SELECT
                    l.nro_legajo,
                    l.apellido_alumno,
...
function estado_alumno_resolve_escuela_activa(
    ...
    return isset($escuelas[$requested]) ? $requested : 'CJ';
}
...
function estado_alumno_cargar_estado_cuenta_alumnos(
    ...
    if (!$ambitoComplejo) {
        $where[] = 'UPPER(RIGHT(TRIM(l.curso), 2)) = ?';
        $types .= 's';
...
function estado_alumno_cargar_emails_alumnos(
    ...
    estado_alumno_stmt_bind($stmt, 's', [$escuelaCodigo]);
    $stmt->execute();
    $filas = estado_alumno_fetch_all_from_stmt($stmt);
    $stmt->close();
