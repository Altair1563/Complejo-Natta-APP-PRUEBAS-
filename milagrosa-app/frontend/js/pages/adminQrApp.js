...
export function initAdminQrApp(appUrl, showToast) {
  ...
  document.getElementById('btnDescargarQr')?.addEventListener('click', () => {
    ...
    link.download = 'milagrosa-app-qr.png';
    link.href = img.src;
    link.click();
  ...
  document.getElementById('btnCompartirQr')?.addEventListener('click', async () => {
    ...
    if (navigator.share && img?.src) {
      try {
        ...
        const file = new File([blob], 'milagrosa-app-qr.png', { type: blob.type || 'image/png' });
        const shareData = {
          title: 'App Jardin de Infantes La Milagrosa',
          text: `Accedé a la APP de Jardin de Infantes La Milagrosa: ${appUrl}`,
          url: appUrl,
        };
