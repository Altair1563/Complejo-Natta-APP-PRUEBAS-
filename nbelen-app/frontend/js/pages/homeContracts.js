...
/** Logos de nivel junto a LogoNbelen en encabezado de contratos. */
const ESCUELA_LOGO_FILES = {
  JB: 'LogoPinocho.png',
  PB: 'LogoINB-P.png',
  SB: 'LogoINB-S.png',
};

/** Letra final del curso → código de escuela (Jardín/Primaria/Secundaria). */
const ESCUELA_POR_LETRA_FINAL = {
  J: 'JB',
  P: 'PB',
  S: 'SB',
  N: 'JB', // NAN ingresos nuevos
};

function escuelaCodigoFromCurso(curso) {
  const c = String(curso ?? '').trim().toUpperCase();
  if (!c) return '';
  const porLetra = ESCUELA_POR_LETRA_FINAL[c.slice(-1)];
  if (porLetra) return porLetra;
  if (c.length < 2) return '';
  return c.slice(-2);
}

function escuelaLogoUrl(codigo) {
  const code = String(codigo ?? '').trim().toUpperCase();
  const file = ESCUELA_LOGO_FILES[code];
  return file ? `./assets/img/${file}` : '';
}

function htmlContractModalHeader({ codigoEscuela, curso } = {}) {
  const codigo = String(codigoEscuela ?? '').trim().toUpperCase() || escuelaCodigoFromCurso(curso);
  const escuelaLogo = escuelaLogoUrl(codigo);
  const escuelaImg = escuelaLogo
    ? `<img src="${escapeHtml(escuelaLogo)}" alt="Logo ${escapeHtml(codigo)}" class="contract-modal-header__logo contract-modal-header__logo--escuela" />`
    : '';
  return `
<div class="contract-modal-header">
  <div class="contract-modal-header__logos">
    <img src="./assets/img/LogoNbelen.jpg" alt="Instituto Nuestra Señora de Itati" class="contract-modal-header__logo contract-modal-header__logo--natta" />
    ${escuelaImg}
  </div>
...
const CONTRACT_DOMICILIO_ELECTRONICO_INSTITUCION = 'cobranzasinb@gmail.com';
