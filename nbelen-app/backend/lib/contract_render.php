...
/**
 * Ruta del template HTML según contract_version (ej. Contrato_JB_2027_v1).
 */
...
/** Códigos de nivel con logo propio junto a LogoNbelen. */
function contrato_escuela_logo_codes(): array
{
    return array_keys(contrato_escuela_logo_files());
}

/**
 * Archivo de logo por código de nivel (JB / PB / SB) en assets/img/.
 *
 * @return array<string, string>
 */
function contrato_escuela_logo_files(): array
{
    return [
        'JB' => 'LogoPinocho.png',
        'PB' => 'LogoINB-P.png',
...
function contrato_escuela_logo_file(string $codigo): ?string
{
    $codigo = mb_strtoupper(trim($codigo), 'UTF-8');
    $map = contrato_escuela_logo_files();
...
/**
 * Bloque de logos (institución + escuela) con URLs absolutas para impresión y vista web.
 */
function contrato_html_logos_header(array $data): string
{
    ...
    $logoFile = tenant_logo_file();
    $logoAlt = (string)(tenant_branding()['contract_logo_alt'] ?? tenant_name_legal());
    $nattaSrc = $base . rawurlencode($logoFile);
    $escuelaImg = '';
    $escuelaLogoFile = contrato_escuela_logo_file($codigo);
    if ($escuelaLogoFile !== null) {
        $escuelaSrc = $base . rawurlencode($escuelaLogoFile);
    ...
    return '<div class="contract-doc-logos">'
        ...
        . '" alt="' . htmlspecialchars($logoAlt, ENT_QUOTES, 'UTF-8') . '" class="contract-doc-logo contract-doc-logo--natta" />'
        . $escuelaImg
        . '</div>'
        . $infoHtml
        . '</div>';
