 ...
 *   initNotificationsPanel();                                // GET sin body
 *   initNotificationsPanel(nroFamilia, csrfToken);          // POST con nro_familia
 */
import {
  AJAX_NOTIFICACIONES,
...
export function initNotificationsPanel(nroFamilia, csrfToken = '') {
  ...
  async function cargarNotificaciones() {
    ...
    try {
      ...
      if (nroFamilia) {
        ...
        form.append('nro_familia', nroFamilia);
        response = await fetch(AJAX_NOTIFICACIONES, { method: 'POST', body: form });
      } else {
        response = await fetch(AJAX_NOTIFICACIONES);
      }
